...
export function buildGrokMessages(
  ...
  const extra = live
    ? `\n${[
        ...
        `<text_anchors>${JSON.stringify(live.textRegions ?? [])}</text_anchors>`,
        "For PDF highlights use the measured text_anchors bbox for the exact quoted fragment. Prefer highlighting the relevant given over covering it with a pointer label. Never point at a formula absent from this page; write it on the board instead. Anchors are normalized to the full PDF page and remain valid at every zoom.",
