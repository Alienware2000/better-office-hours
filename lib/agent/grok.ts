...
import { needsSetupPicture, SETUP_PICTURE_HINT } from "./visual-help";
import {
  buildContextBlock,
  buildVoiceNote,
  DEEP_TURN,
...
export function buildGrokMessages(
  ...
  const setupPicture = needsSetupPicture(history.filter(message => message.role === 'user').at(-1)?.content ?? '');
  const visual = setupPicture ? `\n<setup_picture>${SETUP_PICTURE_HINT}</setup_picture>` : wantsVisualHelp(history)
...
export async function* streamGrok(
  ...
  if (visualRepair) messages.push({ role: 'system', content: (needsSetupPicture(history.filter(message => message.role === 'user').at(-1)?.content ?? '') ? `${SETUP_PICTURE_HINT} This is a silent repair: return geometry or animation, not another formula-only note. ` : '') + 'Compose only the missing whiteboard visual for the last assistant explanation and the student request. Return [BOARD open] and at most five compact valid DRAW commands, with no spoken text. This recovery lane makes a static conceptual sketch, not another animation attempt. If the explanation promised motion, show a few labeled states or directions in a clear still diagram. Never claim the still diagram is moving. Do not repeat the explanation or ask a question. Use a general diagram, relationship, or conceptual example appropriate to the subject. Do not calculate or label any new numeric result or a graded answer. Do not add missing givens, pretend a lecture was retrieved, clear student work, or use a scripted fixture. Keep labels short and diagram geometry clear of text. Use only DRAW ops text, line, arrow, curve, circle, axes. For a box use a closed curve, repeating each corner for straight edges. Put circle positions in center:{x,y}. If the explanation cannot be illustrated responsibly, return nothing.' });
