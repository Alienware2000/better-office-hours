...
- Sessions keeps separate pieces of work in this browser. New session saves the current desk before opening a blank one; putting away a homework PDF uses the same path. Earlier sessions remain available to resume, rename, export, or explicitly delete. Refresh reopens the selected session paused, including conversation memory, board pages and ink, animation state, and PDF annotations/page/zoom. No microphone starts until a user action.
