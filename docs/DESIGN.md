...
Implemented: the desk, custom SVG drawing/animation, PDF ink and pointer, custom STT/Grok/TTS voice loop, live snapshots, verified Yale Google authentication, and server-only Supabase adapters for owned course context and idempotent recaps. The database APIs remain inactive until a Supabase project is configured. Not yet implemented: voice-loop retrieval, the spoken recap close flow, live Canvas/Grok Bot collection, durable PDF storage, and verified Apple Pencil support. Descriptions of those capabilities below are targets unless explicitly listed as implemented here.

Canvas/Grok Bot is the future course-context layer so the tutor already knows the student's classes. David has assigned the live collector integration, and `/api/ingest` now provides its authenticated server boundary. The actual Canvas collection and Duo flow are not yet running. Manual attachments remain usable independently. Broader hackathon scope below is a target backlog, not a claim that every layer is live.
