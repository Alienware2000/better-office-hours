...
David's visual direction (2026-09-11): retain the simple paper diagrams and compact toolbar. Prioritize an annotated explanation that develops with speech: readable labels, short callouts, consistent meaningful color, highlighted features, and compact equations when pedagogically appropriate. Sketchplanations is inspiration for explanatory clarity and annotation, not a requirement to reproduce its illustrations or characters. A circle can stand for a rocket; elaborate silhouettes are optional. The drawing, writing, and annotations should help the learner follow the reasoning like a professor's board. Keep unknown relationships unrevealed until the appropriate teaching step.

Default to using the board whenever the tutor explains, walks through a question, or introduces a situation. Develop a diagram, annotate the existing picture, or add a useful written step as it speaks; animate when change matters. Quick confirmations and short questions may stay verbal when the learner already has the picture. A learner who has selected a problem but cannot start needs an orienting setup, not another general question about what they tried. Do not show or narrate the outcome of the prediction being asked.

Cadence refinement (2026-09-11): the amount of support follows the learner's progress. A correct small answer can receive a short confirmation or invitation to continue, without another diagram, repeated givens, or a forced quiz. New explanations still develop visually. Avoid spoken waiting lines on every turn. When the tutor begins a new working page, bring it into view automatically; the learner should not have to discover it by scrolling. Manual history browsing remains available, and additions to the same page do not pull them away from it.
...
David's 2026-09-11 extension: preserve separate sessions across refresh, resume their conversation and visual state, and export transcripts plus structured session data for analysis. This overrides the earlier disposable-session/no-library decision. The first slice uses local browser storage; authenticated cloud storage and cross-session learner summaries require coordinated integration.
...
- Leave parks the current desk and returns to the lobby with a clean active context. Homework and concept histories remain separate. Re-entering a desk restores its work without exposing the other desk's transcript or PDF to the tutor.
- Sessions keeps separate pieces of work in this browser. A conversation creates its saved entry automatically when it begins; an untouched start screen stays an unsaved draft. The title comes from the conversation or its board topic and can be renamed. Put session navigation in a reserved header above the desk, with a searchable, date-grouped list and per-session export options. Leave saves the conversation and returns to a clean draft. New session saves the current desk before opening a blank one; putting away a homework PDF uses the same path. Earlier sessions remain available to resume, rename, export, or explicitly delete. Refresh reopens the selected session paused, including conversation memory, board pages and ink, animation state, and PDF annotations/page/zoom. No microphone starts until a user action.
- Captions retain the spoken session and remain selectable/exportable. Text export carries timestamps and roles; JSON also carries desk history, board/ink/motion state, attachment references, and local request/playback timing records. These are sentence-playback captions, not word-aligned recordings. Saves/exports contain no microphone recording or hidden course solutions; PDF bytes remain on the demo server. This is user-controlled local recovery, not automatic analytics collection or cross-device account storage.
