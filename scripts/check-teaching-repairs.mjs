...
for (const [written, spoken] of [
  ...
  ['Look at page 1.', 'Look at page one.'],
  ['Problem 21. Now use -9.81 m/s².', 'Problem twenty one. Now use negative 9.81 meters per second squared.'],
  ['See equation 1.2.', 'See equation 1.2.'],
...
const raw = '[TEACH move=explain visual=notes] Here is the relation. [BOARD open][DRAW {"op":"text","id":"eq","at":{"x":0.5,"y":0.3},"text":"F = ma"}] What changes?';
...
console.log('PASS: normalized circle geometry.');
...
const {boardProvenance,asLiveBoard,describeBoard} = load('lib/whiteboard/live-board.ts');
store.applyDrawCommands([{op:'text',id:'topic',at:{x:.5,y:.12},text:'Phase 1: engine on'}, {op:'text',id:'given-1',at:{x:.5,y:.46},text:'a = +3 m/s²'}]);
const initial = boardProvenance(store.getBoardState());
assert.equal(initial.studentStrokeCount,0);
assert.equal(initial.tutorItems[1].text,'a = +3 m/s²');
assert.equal(initial.tutorItems[1].status,'entering','Pending drawing is still authored by the tutor');
store.markGroupShown('given-1');
assert.equal(boardProvenance(store.getBoardState()).tutorItems[1].status,'visible');
store.addStudentStroke({id:'student',tool:'pen',color:'ink',points:[{x:.2,y:.9},{x:.3,y:.9}]});
const ink = structuredClone(store.getBoardState().student);
store.applyDrawCommands([{op:'text',id:'topic',at:{x:.5,y:.12},text:'Phase 2: engine off'}]);
assert.equal(store.getBoardState().groups.length,1,'A heading change cannot relabel old givens as a new phase');
assert.deepEqual(store.getBoardState().earlierPages[0].student,ink,'Earlier student work stays on its original page');
assert.equal(store.getBoardState().student.length,0,'A new working page starts with its own ink');
store.applyDrawCommands([{op:'text',id:'given-1',at:{x:.5,y:.46},text:'a = ?'}]);
store.applyDrawCommands([{op:'text',id:'topic',at:{x:.5,y:.12},text:'Phase 2: engine off'}]);
assert.equal(store.getBoardState().groups.length,2,'An unchanged heading does not erase current notes');
const live = asLiveBoard({...boardProvenance(store.getBoardState()),open:true,imageUrl:'',studentShapesSince:''});
assert.equal(live.studentStrokeCount,0);
assert.equal(live.page,2);
assert.equal(live.earlierPages[0].page,1);
assert.ok(live.revision > initial.revision);
assert.equal(live.tutorItems[1].text,'a = ?');
assert.ok(describeBoard(live).includes('generated by YOU'));
store.addStudentStroke(ink[0]);
store.applyDrawCommands([{op:'clear'}]);
assert.deepEqual(store.getBoardState().student,ink,'Tutor clear never removes student work');
assert.equal(store.getBoardState().groups.length,0);
console.log('PASS: board provenance, entering/visible state, revisions, phase replacement, idempotent headings, and ink-safe clear.');

const {SpeechAudioCapture}=load('app/(session)/voice/audio-capture.ts');
const capture=new SpeechAudioCapture();
for(let i=0;i<20;i++)capture.push(new Float32Array(512).fill(i===19?-.5:0));
capture.start(); capture.push(new Float32Array(512).fill(.5));
const audio=new DataView(await capture.finish().arrayBuffer());
assert.equal(audio.getUint32(24,true),16000);
assert.equal(audio.getUint32(40,true),11*512*2,'Bounded 320ms pre-roll plus the captured frame');
assert.equal(audio.getInt16(44+9*512*2,true),-16384,'Keep the opening sound from before detection');
assert.equal(audio.getInt16(44+10*512*2,true),16383,'Preserve captured samples in order');
capture.push(new Float32Array(512).fill(.8)); capture.discard(); capture.start();
assert.equal((await capture.finish().arrayBuffer()).byteLength,44,'Pause/discard purges prior audio');
const {transcriptionKeyterms,asKeyterms}=load('lib/agent/transcription-context.ts');
assert.ok(transcriptionKeyterms('Which kinematic equation relates velocity and displacement?', '').includes('kinematic'));
assert.ok(!transcriptionKeyterms('The answer is 42 meters.', '').includes('42'));
assert.deepEqual(asKeyterms(['kinematic','kinematic','42','<ignore>','meters per second squared']),['kinematic','meters per second squared']);
console.log('PASS: bounded PCM pre-roll, WAV format/sample order, pause purging, and vocabulary-only STT context.');
