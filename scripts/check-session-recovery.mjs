...
// Fresh browser profile, synthetic transcript/board/PDF, microphone/model disabled.
...
(async()=>{const browser=await chromium.launch({channel:'chrome',headless:true});try{
 ...
 await p.getByRole('button',{name:'Sessions',exact:true}).click();await p.getByRole('textbox',{name:'Current session'}).fill('Motion and components');await p.getByRole('button',{name:'Rename',exact:true}).click();await p.waitForTimeout(400);
 const download=p.waitForEvent('download');await p.getByRole('button',{name:'Export session JSON',exact:true}).click();const exported=await download;const d=JSON.parse(fs.readFileSync(await exported.path()));assert.equal(d.title,'Motion and components');assert.equal(d.transcript.length,2);assert.equal(d.voice.current.board.animation.id,'sample-motion');assert.equal(d.pdf['test-pdf'].ink.present[0].id,'pdf-ink');
 await p.screenshot({path:path.join(output, 'boh-sessions-library.png')});
 await p.getByRole('button',{name:'+ New session',exact:true}).click();await p.getByRole('button',{name:'Homework',exact:true}).waitFor();assert.equal(await p.getByRole('region',{name:'Session transcript'}).count(),0);
 await p.getByRole('button',{name:'Sessions',exact:true}).click();await p.getByRole('textbox',{name:'Current session'}).fill('Another piece of work');await p.getByRole('button',{name:'Rename',exact:true}).click();await p.waitForTimeout(250);
 await p.getByRole('button',{name:/Motion and components/}).click();await p.getByRole('region',{name:'Session transcript'}).waitFor();await p.getByRole('button',{name:'Notes',exact:true}).click();await p.locator('[data-page="0"]').waitFor();await p.waitForTimeout(400);await p.screenshot({path:path.join(output, 'boh-restored-pdf.png')});
 ...
 await p.setViewportSize({width:700,height:950});await p.screenshot({path:path.join(output, 'boh-sessions-compact.png')});await p.getByRole('button',{name:'Sessions',exact:true}).click();await p.screenshot({path:path.join(output, 'boh-sessions-library-compact.png')});
 const textDownload=p.waitForEvent('download');await p.getByRole('button',{name:'Export transcript',exact:true}).click();
 assert.ok(fs.readFileSync(await (await textDownload).path(),'utf8').includes('Tutor: The two directions share the same time.'));
 await p.getByRole('button',{name:'Delete Another piece of work',exact:true}).click();
 await p.locator('.session-delete-confirm').getByRole('button',{name:'Delete',exact:true}).click();
 ...
 await p.getByRole('textbox',{name:'Current session'}).fill('Stale edit');await p.getByRole('button',{name:'Rename',exact:true}).click();
 ...
 assert.equal(api.filter(u=>!u.endsWith('/health')).length,0);assert.deepEqual(errors,[]);console.log('PASS: real IndexedDB restore, separate new session, rename, history/board/ink/animation/PDF-state export, reload twice, editable PDF ink/zoom/page recovery, stale-tab conflict protection, desktop/compact, no mic/model calls.');
